<p align="center">

  <img alt="ThriftVerse" title="ThriftVerse" src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/Logo.svg" width="200">
</p>

<p align="center">
  A Social-Driven Online Thrifting Ecosystem Where Sustainability Meets Individuality
</p>

<p align="center">

  <img src="https://img.shields.io/badge/React-18.2.0-61DAFB?logo=react" alt="React" />
  <img src="https://img.shields.io/badge/Vite-4.4.0-646CFF?logo=vite" alt="Vite" />
  <img src="https://img.shields.io/badge/Supabase-Backend-3ECF8E?logo=supabase" alt="Supabase" />
</p>

---

# ThriftVerse 👗♻️

**ThriftVerse** is a digital platform for buying, selling, swapping, and discovering secondhand fashion in a more interactive and trusted way. Unlike traditional e-commerce platforms, ThriftVerse combines e-commerce, social interaction, creative fashion content, and sustainable shopping behavior — all in one place.

Built to address the growing fast fashion crisis, ThriftVerse empowers Gen Z and young adults to thrift smarter, connect with sellers directly, and participate in a more sustainable fashion ecosystem.

---

## 📌 Overview

| Field        | Details                                                                         |
|--------------|---------------------------------------------------------------------------------|
| **Name**     | ThriftVerse                                                                     |
| **Type**     | Social-Driven Second-Hand Fashion Marketplace                                   |
| **Frontend** | React 18 + Vite + React Router DOM                                              |
| **Backend**  | Supabase (Auth, Realtime, Storage)                                              |
| **Database** | PostgreSQL (via Supabase)                                                       |
| **Target Users** | Gen Z, young adults, fashion enthusiasts, sustainable lifestyle communities |

---

## 🔥 Key Features

- 🛍️ **Buy & Sell Thrift Items** — Browse and purchase curated secondhand fashion across categories: Men, Women, Kids, Accessories, Shoes.

- 🔁 **Barter System** — Users can offer their own listed products as a trade, with optional partial cash payment if item values differ.

- 🎬 **ThriftVid (Short-Form Videos)** — Sellers can showcase thrift outfits, styling ideas, and product previews in a TikTok-style short-form video feed with comments.

- 💬 **In-App Chat** — Buyers and sellers can chat directly to negotiate, ask questions, and send barter offers in real-time.

- 🛒 **Cart & Checkout** — Full cart management with quantity controls, multiple shipping options (Regular, Express, Same Day), and multiple payment methods (BCA VA, BRI VA, GoPay, QRIS, Indomaret, COD).

- 🏪 **Seller Dashboard** — Sellers get a full dashboard to manage listings, upload products, track incoming orders, monitor expedition status, and view buyer chats.

- 📦 **Purchases & Order Tracking** — Buyers can track their orders through status stages: Pending → Diproses → Dikirim → Selesai, and confirm receipt when delivered.

- 👤 **Profile & Shop Setup** — Users can set up a personal profile and shop, including shop name and branding, required before accessing the seller dashboard.

- 🔐 **Authentication** — Secure email-based sign-up and login via Supabase Auth, with protected routes for authenticated users.

---

## 🎯 Problem Solved

The fast fashion industry generates millions of tons of waste annually as clothing is discarded after only a few uses. ThriftVerse tackles this by:

- **Reducing waste** — Making it easy to resell and swap clothes that are still wearable.
- **Building community** — Turning transactional thrifting into a social experience through chats, ThriftVid, and barter.
- **Creating trust** — Offering a structured, verified platform compared to informal secondhand channels.
- **Promoting sustainability** — Encouraging reuse and reducing unnecessary new purchases through a barter-first mindset.

---

## 💻 Mockups & Screenshots

### 🏠 Home Page


<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/Home%20Page.png" width="700" alt="ThriftVerse Home Page">
</p>

---

### 🔐 Authentication

- **Login Modal**

<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/Login%20Modal.png" width="700" alt="Login Modal">
</p>

- **Register Page**


<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/Register%20Page.png" width="700" alt="Register Page">
</p>

---

### 🎬 ThriftVid


<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/ThiftVid.png" width="700" alt="ThriftVid Feed">
</p>

<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/ThriftVid%20Viewer.png" width="700" alt="ThriftVid Viewer with Comments">
</p>

---

### 🛍️ Product Browsing & Detail

- **Product Carousel (Home)**

<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/Product%20Carousel.png" width="700" alt="Product Carousel">
</p>

- **Product Detail Page**

<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/Product%20Detail.png" width="700" alt="Product Detail Page">
</p>

- **Barter Offer**

<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/Barter%20Offer.png" width="700" alt="Barter Offer Picker">
</p>

---

### 🛒 Cart & Checkout

- **Cart Page**

<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/Cart%20Page.png" width="700" alt="Cart Page">
</p>

- **Checkout Page**

<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/Checkout%20Page%201.png" width="700" alt="Checkout Page">
</p>
<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/Checkout%20Page%202.png" width="700" alt="Checkout Page">
</p>


- **Order Confirmation**

<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/Order%20Confirmation.png" width="700" alt="Order Confirmation">
</p>

---

### 💬 In-App Chat

<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/Barter%20Chat%20Card%201.png" width="700" alt="Chat Page">
</p>

<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/Barter%20Chat%20Card%202.png" width="700" alt="Chat Page">
</p>

---

### 🏪 Seller Dashboard

- **Shop Setup (Onboarding)**

<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/Shop%20Setup.png" width="700" alt="Shop Setup Page">
</p>

- **Seller Overview**

<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/Seller%20Overview.png" width="700" alt="Seller Dashboard Overview">
</p>

- **My Listings**

<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/Seller%20Listings.png" width="700" alt="Seller My Listings">
</p>

- **Upload Product**

<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/Seller%20Upload%20Product.png" width="700" alt="Upload Product Form">
</p>

- **Incoming Orders**

<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/Seller%20Incoming%20Orders.png" width="700" alt="Seller Incoming Orders">
</p>

- **Buyer Chats (from Seller Side)**

<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/Seller%20Buyer%20Chats.png" width="700" alt="Seller Buyer Chats">
</p>

---

### 📦 Purchases & Order Tracking (Buyer)

- **My Purchases**

<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/My%20Purchases.png" width="700" alt="My Purchases">
</p>

- **Expedition Tracker**

<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/Expedition%20Tracker.png" width="700" alt="Expedition Tracker">
</p>

---

### 👤 Profile Page

<p align="center">
  <img src="https://ik.imagekit.io/nauvalihsan/Thriftverse%20Assets/Profile%20Page.png" width="700" alt="Profile Page">
</p>

---

## 🌍 Feature Demo

- **Register & Login**

<p align="center">
  <img src="src/assets/Demo Register.gif" width="700" alt="Demo Register & Login">
</p>
<p align="center">
  <img src="src/assets/Demo Login.gif" width="700" alt="Demo Register & Login">
</p>


- **Browse & Buy a Product**

<p align="center">
  <img src="src/assets/Demo Buy.gif" width="700" alt="Demo Browse and Buy">
</p>

- **Barter a Product**

<p align="center">
  <img src="src/assets/Demo Barter Flow.gif" width="700" alt="Demo Barter Flow">
</p>

- **Seller Upload & Manage Product**

<p align="center">
  <img src="src/assets/Demo Seller Upload Product.gif" width="700" alt="Demo Seller Upload Product">
</p>

- **Track Order (Buyer)**

<p align="center">
  <img src="src/assets/Demo Track Order.gif" width="700" alt="Demo Track Order">
</p>

---

## ⚙️ Technology Stack

### 💡 Frontend

| Technology          | Purpose                                       |
|---------------------|-----------------------------------------------|
| _React 18_          | UI component library                          |
| _Vite 4_            | Build tool and development server             |
| _React Router DOM 6_ | Client-side routing and navigation           |
| _CSS Modules_       | Component-scoped styling                      |

### 🗄️ Backend & Database

| Technology          | Purpose                                       |
|---------------------|-----------------------------------------------|
| _Supabase_          | Backend-as-a-Service (Auth, Realtime, Storage)|
| _PostgreSQL_        | Relational database (via Supabase)            |
| _Supabase Realtime_ | Live chat and order status subscriptions      |

### 🏗️ Architecture

ThriftVerse follows a **client-server architecture**:
- **Frontend** handles all UI interactions and state management.
- **Supabase** acts as the backend, handling authentication, product data, order transactions, and real-time chat.
- **PostgreSQL** stores users, products, orders, barter offers, and chat messages.

---

## ✅ Prerequisites

Make sure the following are installed on your machine before running ThriftVerse:

- [Node.js 16+](https://nodejs.org/)
- [Git](https://git-scm.com/)
- A [Supabase](https://supabase.com/) account and project

---

## 🚀 How to Run Locally

### 1. Clone the Repository

```bash
git clone https://github.com/your-username/thriftverse.git
cd thriftverse
```

### 2. Install Dependencies

```bash
npm install
```

### 3. Configure Environment Variables

Create a `.env.local` file in the root directory and fill in your Supabase credentials:

```env
VITE_SUPABASE_URL=your_supabase_project_url
VITE_SUPABASE_ANON_KEY=your_supabase_anon_key
```

> You can find these values in your Supabase project dashboard under **Settings → API**.


### 4. Start the Development Server

```bash
npm run dev
```

The app will be available at `http://localhost:5173`.

### 5. Build for Production

```bash
npm run build
```

---

## 📁 Project Structure

```
thriftverse/
├── public/                  # Static assets
├── src/
│   ├── assets/              # Images and SVGs
│   ├── components/          # Reusable UI components
│   │   ├── seller/          # Seller-specific components
│   │   ├── Navbar.jsx       # Top navigation bar
│   │   ├── HeroBanner.jsx   # Hero section on Home
│   │   ├── WindowShorts.jsx # ThriftVid short-form video feed
│   │   ├── ProductCarousel.jsx  # Product listing carousel
│   │   ├── BarterCard.jsx   # Barter offer card in chat
│   │   ├── LoginPage.jsx    # Login modal
│   │   └── RegisterPage.jsx # Registration page
│   ├── data/                # Static data (shorts, products)
│   ├── lib/                 # Supabase service functions
│   │   ├── supabase.js      # Supabase client initialization
│   │   ├── products.js      # Product queries
│   │   ├── orders.js        # Order management
│   │   ├── chat.js          # Chat & barter logic
│   │   ├── profiles.js      # User profile & shop functions
│   │   └── shortsComments.js # Comments for ThriftVid
│   ├── pages/               # Page-level components
│   │   ├── HomePage.jsx     # Main landing page
│   │   ├── ProductDetailPage.jsx  # Single product view
│   │   ├── CartPage.jsx     # Shopping cart
│   │   ├── CheckoutPage.jsx # Checkout flow
│   │   ├── PurchasesPage.jsx # Buyer order tracking
│   │   ├── SellerPage.jsx   # Seller dashboard
│   │   ├── ShopSetupPage.jsx # Seller onboarding
│   │   ├── ProfilePage.jsx  # User profile
│   │   └── ChatPage.jsx     # In-app messaging
│   ├── App.jsx              # Root component with routing
│   └── main.jsx             # App entry point
├── .env.local               # Environment variables (not committed)
├── package.json
└── vite.config.js
```

---

## 👥 Contact

Feel free to reach out if you need help setting it up!


| NIM            | Name                                         |
|----------------|----------------------------------------------|
| `2802565852`   | M Akbar Sandrego                             |
| `2802439643`   | Jason Clein                                  |
| `2802541972`   | Daniel Isacc Francis Wibowo                  |
| `2802552370`   | Andi Muhammad Nauval Ihsan Fairell           |
| `2802461126`   | Yohanes Dion Cong                            |

---

> Made with ♻️ for a more sustainable fashion future — **ThriftVerse**